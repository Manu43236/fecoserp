...
@RestController
@RequestMapping("/api/v1/masters/product-categories")
...
public class ProductCategoryController {
    ...
    public ResponseEntity<ApiResponse<ProductCategoryResponse>> create(@Valid @RequestBody PumpTypeRequest req) {
        return ResponseEntity.ok(ApiResponse.ok(service.create(req)));
    }

    @PatchMapping("/{id}/toggle")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<ProductCategoryResponse>> toggle(@PathVariable UUID id) {
