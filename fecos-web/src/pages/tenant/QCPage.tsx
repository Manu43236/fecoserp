...
import { X, Plus, FlaskConical, ChevronRight, CheckCircle2, XCircle, TestTube, Package, Warehouse } from 'lucide-react'
import toast from 'react-hot-toast'
import { rawQcApi } from '@/api/rawQc'
import type { RawMaterialBatch, RawMaterialBatchPayload, RawMaterialResultPayload, RawMaterialStatus } from '@/api/rawQc'
...
function statusBadge(status: RawMaterialStatus | FinishedProductStatus) {
...
function PanelHeader({ icon: Icon, title, sub, onClose }: { icon: React.ElementType; title: string; sub?: string; onClose: () => void }) {
  return (
...
function RawCreatePanel({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState<RawMaterialBatchPayload>({
    supplierName: '', materialName: '', quantity: 0, unit: 'Gallon',
    receivedDate: new Date().toISOString().split('T')[0], supplierLotNumber: '',
  })
  const mutation = useMutation({
    mutationFn: (data: RawMaterialBatchPayload) => rawQcApi.create(data),
    onSuccess: () => { toast.success('Batch logged'); onSaved() },
    onError:   () => toast.error('Failed to create batch'),
  })
  const set = (k: keyof RawMaterialBatchPayload, v: string | number) => setForm(f => ({ ...f, [k]: v }))
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.supplierName || !form.materialName || !form.quantity || !form.unit || !form.receivedDate)
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-[440px] h-full bg-white shadow-xl flex flex-col">
        ...
        <form onSubmit={submit} className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          <div>
            ...
            <input value={form.supplierName} onChange={e => set('supplierName', e.target.value)}
              ...
              placeholder="e.g. ChemSource Inc." />
          </div>
          <div>
            ...
            <input value={form.materialName} onChange={e => set('materialName', e.target.value)}
              className="w-full h-9 px-3 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)]"
              placeholder="e.g. Corrosion Inhibitor Base" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <SearchableDropdown value={form.unit} onChange={v => set('unit', v ?? 'Gallon')} showClear={false}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Supplier Lot # <span className="text-gray-400">(optional)</span></label>
            <input value={form.supplierLotNumber || ''} onChange={e => set('supplierLotNumber', e.target.value)}
              className="w-full h-9 px-3 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)]"
              placeholder="Lot number from supplier label" />
          </div>
        </form>
        <div className="px-5 py-4 flex gap-3 shrink-0" style={{ borderTop: '1px solid rgba(0,0,0,0.07)' }}>
          ...
          <PrimaryBtn onClick={submit} disabled={mutation.isPending}>
...
function RawResultsPanel({ batch, onClose, onSaved }: { batch: RawMaterialBatch; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState<RawMaterialResultPayload>({
    appearance: '', colorOk: undefined, odor: '', ph: undefined,
    specificGravity: undefined, notes: '', result: 'PASSED',
  })
  const mutation = useMutation({
    mutationFn: (data: RawMaterialResultPayload) => rawQcApi.enterResults(batch.id, data),
    onSuccess: () => { toast.success('Results saved'); onSaved() },
    onError:   () => toast.error('Failed to save results'),
  })
  const set = (k: keyof RawMaterialResultPayload, v: string | number | boolean | undefined) =>
    setForm(f => ({ ...f, [k]: v }))
  return (
      ...
      <div className="relative w-[440px] h-full bg-white shadow-xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          <div>
            ...
            <SearchableDropdown value={form.appearance || null} onChange={v => set('appearance', v ?? '')}
              placeholder="Select…" showClear={false} options={APPEARANCE_OPTIONS.map(o => ({ value: o, label: o }))} />
          </div>
          <div>
          ...
          <div>
            ...
            <SearchableDropdown value={form.odor || null} onChange={v => set('odor', v ?? '')}
              placeholder="Select…" showClear={false} options={ODOR_OPTIONS.map(o => ({ value: o, label: o }))} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">pH</label>
              <input type="number" step="0.01" min="0" max="14" value={form.ph ?? ''}
                onChange={e => set('ph', e.target.value ? parseFloat(e.target.value) : undefined)}
                className="w-full h-9 px-3 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)]"
                placeholder="0–14" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Specific Gravity</label>
              <input type="number" step="0.0001" min="0" value={form.specificGravity ?? ''}
                onChange={e => set('specificGravity', e.target.value ? parseFloat(e.target.value) : undefined)}
                className="w-full h-9 px-3 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)]"
...
function RawBatchDrawer({ id, onClose }: { id: string; onClose: () => void }) {
  const qc = useQueryClient()
  const [showResults, setShowResults] = useState(false)
  const { data, isLoading } = useQuery({ queryKey: ['raw-qc', id], queryFn: () => rawQcApi.get(id) })
  const startMutation = useMutation({
    mutationFn: () => rawQcApi.startTesting(id),
    onSuccess:  () => { toast.success('Testing started'); qc.invalidateQueries({ queryKey: ['raw-qc'] }) },
    onError:    () => toast.error('Failed to start testing'),
  })
  const batch = data?.data?.data
  if (isLoading || !batch) return null
  const field = (label: string, value: React.ReactNode) => (
    <div><p className="text-xs text-gray-500 mb-0.5">{label}</p><p className="text-sm font-medium text-gray-900">{value ?? '—'}</p></div>
  )
  return (
    <>
        ...
        <div className="relative w-[600px] h-full bg-white shadow-xl flex flex-col">
          <div className="flex items-center gap-3 px-6 py-4 shrink-0" style={{ borderBottom: '1px solid rgba(0,0,0,0.07)' }}>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{batch.materialName}</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {field('Supplier', batch.supplierName)}
                {field('Material', batch.materialName)}
                {field('Quantity', `${batch.quantity} ${batch.unit}`)}
                {field('Received Date', batch.receivedDate)}
                {field('Supplier Lot #', batch.supplierLotNumber)}
              </div>
            </div>
            {(batch.status === 'PASSED' || batch.status === 'FAILED' || batch.status === 'IN_PROGRESS') && (
              <div>
            ...
            {batch.status === 'PASSED' && (
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-green-50 border border-green-100">
                ...
                <p className="text-sm font-medium text-green-800">This batch passed QC and is cleared for blending.</p>
              </div>
            )}
            {batch.status === 'FAILED' && (
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-red-50 border border-red-100">
                <XCircle size={18} className="text-red-600 shrink-0" />
                <p className="text-sm font-medium text-red-800">This batch failed QC. Quarantine and contact supplier.</p>
              </div>
            )}
          </div>
          <div className="px-6 py-4 flex gap-3 shrink-0" style={{ borderTop: '1px solid rgba(0,0,0,0.07)' }}>
            {batch.status === 'PENDING' && (
              <PrimaryBtn onClick={() => startMutation.mutate()} disabled={startMutation.isPending}>
                {startMutation.isPending ? 'Starting…' : 'Start Testing'}
              </PrimaryBtn>
            )}
            {batch.status === 'IN_PROGRESS' && (
              <PrimaryBtn onClick={() => setShowResults(true)}>Enter Results</PrimaryBtn>
            )}
          </div>
        </div>
      </div>
      {showResults && (
        <RawResultsPanel batch={batch} onClose={() => setShowResults(false)}
          onSaved={() => { setShowResults(false); qc.invalidateQueries({ queryKey: ['raw-qc'] }) }} />
      )}
    </>
  )
}
...
function RawMaterialsSection() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['raw-qc', statusFilter, page],
    queryFn:  () => rawQcApi.list({ status: statusFilter === 'ALL' ? undefined : statusFilter, page, size: 20 }),
  })
  const allBatches = data?.data?.data?.content ?? []
  const total      = data?.data?.data?.totalElements ?? 0
  const pages      = data?.data?.data?.totalPages ?? 1
  const batches    = search.trim()
    ? allBatches.filter(b => b.materialName.toLowerCase().includes(search.toLowerCase()) || b.supplierName.toLowerCase().includes(search.toLowerCase()))
    : allBatches
  const refresh = () => qc.invalidateQueries({ queryKey: ['raw-qc'] })

  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search material or supplier…"
          ...
          <div className="w-44">
            <SearchableDropdown value={statusFilter} onChange={v => { setStatusFilter((v ?? 'ALL') as RawMaterialStatus | 'ALL'); setPage(0) }}
              showClear={false} options={STATUS_OPTS} />
          </div>
        </div>
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={7} className="px-4 py-14 text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-1">{total === 0 ? 'Log the first raw material batch to get started' : 'Try adjusting your filters'}</p>
                </td>
              </tr>
            ) : batches.map(b => (
              <tr key={b.id} onClick={() => setSelectedId(b.id)} className="cursor-pointer hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-gray-600">{b.receivedDate}</td>
      ...
      {selectedId && <RawBatchDrawer id={selectedId} onClose={() => { setSelectedId(null); refresh() }} />}
      {showCreate && <RawCreatePanel onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); refresh() }} />}
    </div>
  )
}
...
function MoveToWarehousePanel({ batch, onClose, onMoved }: { batch: FinishedProductBatch; onClose: () => void; onMoved: () => void }) {
  const [warehouseId, setWarehouseId] = useState('')
  const { data: warehouseData } = useQuery({ queryKey: ['warehouses-active'], queryFn: () => warehousesApi.listActive() })
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-[400px] p-6">
        ...
        <div className="mb-5">
          ...
          <SearchableDropdown value={warehouseId || null} onChange={v => setWarehouseId(v ?? '')}
            placeholder="Select warehouse…" showClear={false}
...
function FinishedCreatePanel({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const { data: productData } = useQuery({ queryKey: ['products-all'], queryFn: () => productsApi.list({ size: 200, isActive: true }) })
  ...
  const set = (k: keyof FinishedProductBatchPayload, v: string | number) => setForm(f => ({ ...f, [k]: v }))
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.productId || !form.quantity || !form.unit || !form.blendDate) return toast.error('Fill all required fields')
    mutation.mutate(form)
  }
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-[440px] h-full bg-white shadow-xl flex flex-col">
        ...
        <form onSubmit={submit} className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          <div>
            ...
            <SearchableDropdown value={form.productId || null} onChange={v => set('productId', v ?? '')}
              placeholder="Select product…" showClear={false}
...
function FinishedResultsPanel({ batch, onClose, onSaved }: { batch: FinishedProductBatch; onClose: () => void; onSaved: () => void }) {
...
function FinishedBatchDrawer({ id, onClose }: { id: string; onClose: () => void }) {
  const qc = useQueryClient()
  const [showResults, setShowResults]     = useState(false)
  const [showWarehouse, setShowWarehouse] = useState(false)
  const { data, isLoading } = useQuery({ queryKey: ['finished-qc', id], queryFn: () => finishedQcApi.get(id) })
  const startMutation = useMutation({
  ...
  return (
    <>
          ...
          <div className="px-6 py-4 flex gap-3 shrink-0" style={{ borderTop: '1px solid rgba(0,0,0,0.07)' }}>
            ...
            {batch.status === 'PASSED' && !batch.movedToWarehouse && (
              <button onClick={() => setShowWarehouse(true)}
                className="flex-1 h-10 text-sm font-semibold rounded-lg text-white flex items-center justify-center gap-2 transition-colors"
                style={{ backgroundColor: 'var(--color-primary)' }}>
      ...
      {showResults && (
        <FinishedResultsPanel batch={batch} onClose={() => setShowResults(false)}
...
function FinishedProductSection() {
  const [statusFilter, setStatusFilter] = useState<FinishedProductStatus | 'ALL'>('ALL')
  const [search, setSearch] = useState('')
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const [showCreate, setShowCreate] = useState(false)
  const [page, setPage] = useState(0)
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['finished-qc', statusFilter, page],
    queryFn:  () => finishedQcApi.list({ status: statusFilter === 'ALL' ? undefined : statusFilter, page, size: 20 }),
  })
  const allBatches = data?.data?.data?.content ?? []
  const total      = data?.data?.data?.totalElements ?? 0
  const pages      = data?.data?.data?.totalPages ?? 1
  const batches    = search.trim()
    ? allBatches.filter(b => b.productName.toLowerCase().includes(search.toLowerCase()))
    : allBatches
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search product…"
            className="h-9 px-3 text-sm border border-gray-200 rounded-lg w-56 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)]" />
          <div className="w-44">
            <SearchableDropdown value={statusFilter} onChange={v => { setStatusFilter((v ?? 'ALL') as FinishedProductStatus | 'ALL'); setPage(0) }}
              showClear={false} options={STATUS_OPTS} />
          </div>
        </div>
        <button onClick={() => setShowCreate(true)}
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={7} className="px-4 py-14 text-center">
                  ...
                  <p className="text-sm text-gray-500 font-medium">No batches found</p>
                  <p className="text-xs text-gray-400 mt-1">{total === 0 ? 'Log the first finished product batch to get started' : 'Try adjusting your filters'}</p>
                </td>
              </tr>
            ) : batches.map(b => (
              <tr key={b.id} onClick={() => setSelectedId(b.id)} className="cursor-pointer hover:bg-gray-50 transition-colors">
      ...
      {selectedId && <FinishedBatchDrawer id={selectedId} onClose={() => { setSelectedId(null); refresh() }} />}
      {showCreate && <FinishedCreatePanel onClose={() => setShowCreate(false)} onSaved={() => { setShowCreate(false); refresh() }} />}
    </div>
  )
}
...
const TABS = [
  ...
  { key: 'finished', label: 'Final Product'  },
] as const
...
export default function QCPage() {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      {activeTab === 'raw'      && <RawMaterialsSection />}
      {activeTab === 'finished' && <FinishedProductSection />}
    </div>
  )
}
