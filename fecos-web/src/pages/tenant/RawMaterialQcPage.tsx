...
import { X, Plus, FlaskConical, ChevronRight, CheckCircle2, XCircle, Clock, TestTube } from 'lucide-react'
...
const STATUS_TABS: { label: string; value: RawMaterialStatus | 'ALL' }[] = [
  { label: 'All',         value: 'ALL' },
...
function statusBadge(status: RawMaterialStatus) {
  const map: Record<RawMaterialStatus, { label: string; color: string }> = {
...
function CreatePanel({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const set = (k: keyof RawMaterialBatchPayload, v: string | number) =>
    setForm(f => ({ ...f, [k]: v }))
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-[440px] h-full bg-white shadow-xl flex flex-col">
        ...
        <div className="flex items-center gap-3 px-5 py-4 shrink-0" style={{ borderBottom: '1px solid rgba(0,0,0,0.07)' }}>
          ...
          <h2 className="text-sm font-semibold text-gray-900">Log Raw Material Batch</h2>
        ...
        <div className="px-5 py-4 flex gap-3 shrink-0" style={{ borderTop: '1px solid rgba(0,0,0,0.07)' }}>
          ...
          <button onClick={submit} disabled={mutation.isPending}
            className="flex-1 h-10 text-sm font-semibold rounded-lg text-white transition-colors disabled:opacity-60"
...
function ResultsPanel({ batch, onClose, onSaved }: { batch: RawMaterialBatch; onClose: () => void; onSaved: () => void }) {
      ...
      <div className="relative w-[440px] h-full bg-white shadow-xl flex flex-col">
        <div className="flex items-center gap-3 px-5 py-4 shrink-0" style={{ borderBottom: '1px solid rgba(0,0,0,0.07)' }}>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">{batch.batchNumber} — {batch.materialName}</p>
          </div>
          <button onClick={onClose} className="ml-auto text-gray-400 hover:text-gray-600"><X size={18} /></button>
        </div>
      ...
      {showResults && (
        <ResultsPanel
          ...
          onSaved={() => {
            ...
            qc.invalidateQueries({ queryKey: ['raw-qc'] })
          }}
        />
      )}
    </>
...
export default function RawMaterialQcPage() {
  const [activeTab, setActiveTab] = useState<RawMaterialStatus | 'ALL'>('ALL')
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['raw-qc', activeTab, page],
    queryFn:  () => rawQcApi.list({ status: activeTab === 'ALL' ? undefined : activeTab, page, size: 20 }),
  })
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="bg-white px-6 py-4 flex items-center justify-between shrink-0" style={{ borderBottom: '1px solid rgba(0,0,0,0.07)' }}>
        <div>
          <h1 className="text-xl font-bold text-gray-900">Raw Material QC</h1>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-14 text-center">
                    ...
                    <p className="text-xs text-gray-400 mt-1">Log the first raw material batch to get started</p>
                  </td>
                </tr>
              ) : batches.map((b, i) => (
                <tr key={b.id}
