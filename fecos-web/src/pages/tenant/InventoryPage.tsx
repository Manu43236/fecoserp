...
import {
  Plus, X, Warehouse, BarChart2, ArrowLeftRight, ChevronRight, Search, FlaskConical,
} from 'lucide-react'
...
// ── Section nav ───────────────────────────────────────────────────────────────
type SectionId = 'products' | 'warehouses' | 'stock' | 'transactions'

const SECTIONS: { id: SectionId; label: string; description: string; icon: React.ComponentType<{ size?: number; className?: string }> }[] = [
  { id: 'products',     label: 'Products',     description: 'Product catalog',                   icon: FlaskConical   },
...
function ProductFormPanel({
  ...
  function onProductSubmit(data: ProductFormData) {
    mutation.mutate({
      name:         data.name,
  ...
  const categoryOptions: DropdownOption[] = categories.filter(c => c.isActive).map(c => ({ value: c.name, label: c.name }))
  const unitOptions: DropdownOption[]     = units.filter(u => u.isActive).map(u => ({ value: u.name, label: u.name }))
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative bg-white w-full md:w-[440px] h-full shadow-2xl flex flex-col">
        ...
        <form onSubmit={handleSubmit(onProductSubmit)} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Product Name *</label>
              <input {...register('name')} placeholder="e.g. Corrosion Inhibitor CI-1000"
            ...
            <div className="space-y-1.5">
              ...
              <input {...register('productCode')} placeholder="e.g. CI-1000"
                className="w-full h-10 px-3 text-sm rounded-lg border border-gray-200 outline-none transition focus:ring-2 focus:ring-red-100" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <SearchableDropdown options={categoryOptions} value={watch('category')} onChange={v => setValue('category', v ?? '')} placeholder="Select" showClear={false} />
                {errors.category && <p className="text-xs text-red-500">{errors.category.message}</p>}
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Unit *</label>
                <SearchableDropdown options={unitOptions} value={watch('unit')} onChange={v => setValue('unit', v ?? '')} placeholder="Select" showClear={false} />
            ...
            <div className="space-y-1.5">
              ...
              <input {...register('pricePerUnit')} type="number" step="0.0001" min="0" placeholder="0.00"
                className="w-full h-10 px-3 text-sm rounded-lg border border-gray-200 outline-none transition focus:ring-2 focus:ring-red-100" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
              ...
              <textarea {...register('description')} rows={3} placeholder="Product notes or specs…"
            ...
            {isEdit && (
              <>
                ...
                <div className="space-y-1.5">
                  ...
                  <button type="button" onClick={() => setValue('isActive', !watch('isActive'))}
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative bg-white w-full md:w-[400px] h-full shadow-2xl flex flex-col">
        ...
        <div className="px-5 py-4 shrink-0" style={{ borderTop: '1px solid rgba(0,0,0,0.07)' }}>
          <button onClick={onEdit} className="w-full h-10 text-sm font-semibold text-white rounded-lg transition"
            style={{ backgroundColor: 'var(--color-primary)' }}>Edit Product</button>
        </div>
      </div>
    </div>
  )
...
function ProductsSection({ canEdit }: { canEdit: boolean }) {
  ...
  const categoryOptions: DropdownOption[] = (categories as ProductCategoryRecord[])
    .filter(c => c.isActive).map(c => ({ value: c.name, label: c.name }))
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="w-48">
            <SearchableDropdown options={categoryOptions} value={categoryFilter} onChange={v => { setCategoryFilter(v); setPage(0) }} placeholder="All Categories" showClear />
          </div>
          <div className="w-36">
            <SearchableDropdown options={statusOptions} value={statusFilter} onChange={v => { setStatusFilter(v); setPage(0) }} placeholder="All Statuses" showClear />
          </div>
        </div>
        {canEdit && (
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {products.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-14 text-center text-gray-400 text-sm">
                No products found. {canEdit && 'Create your first product to get started.'}
              </td></tr>
            ) : (products as ProductRecord[]).map(p => (
              <tr key={p.id} onClick={() => setViewing(p)} className="cursor-pointer hover:bg-gray-50 transition-colors">
      ...
      />
      {viewing && !panelOpen && (
        <ProductDrawer product={viewing} onClose={() => setViewing(undefined)} onEdit={() => openEdit(viewing)} />
      )}
    </div>
  )
}
...
export default function InventoryPage() {
  ...
  const [active, setActive] = useState<SectionId>('products')
  ...
  return (
    <div className="flex flex-col h-full min-h-screen">
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        {active === 'products'     && <ProductsSection    canEdit={canEdit} />}
