...
import {
  ...
  Monitor, Smartphone, ChevronRight, ChevronLeft, X, Mail, Phone, Lock, Eye, EyeOff, Users,
} from 'lucide-react'
...
function UserFormPanel({ open, onClose, user }: { open: boolean; onClose: () => void; user?: UserRecord }) {
      ...
      <div className="relative bg-white w-[440px] h-full shadow-2xl flex flex-col">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-gray-900">{isEdit ? 'Edit User' : 'Add User'}</h2>
              <p className="text-xs text-gray-400">{isEdit ? 'Update user details' : 'Create a new account for your team'}</p>
            </div>
          </div>
        ...
        >
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Full Name <span className="text-red-500">*</span></label>
              <input {...form.register('fullName')} placeholder="John Smith"
                className="w-full h-10 px-3 text-sm border border-gray-200 rounded-lg outline-none transition focus:ring-2 focus:ring-red-100" />
              {'fullName' in form.formState.errors && <p className="text-red-500 text-xs">{form.formState.errors.fullName?.message}</p>}
            </div>
            ...
            {!isEdit && (
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Mobile <span className="text-red-500">*</span></label>
                    <div className="flex">
                      <span className="flex items-center px-2.5 h-10 rounded-l-lg border border-r-0 border-gray-200 bg-gray-50 text-xs text-gray-500 font-medium">+1</span>
                      <input
                        type="tel"
                        inputMode="numeric"
                        ...
                        className="flex-1 h-10 px-2.5 text-sm border border-gray-200 rounded-r-lg outline-none transition focus:ring-2 focus:ring-red-100 tracking-wide"
                      />
                    </div>
                    {createForm.formState.errors.mobileNumber && <p className="text-red-500 text-xs">{createForm.formState.errors.mobileNumber.message}</p>}
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">PIN <span className="text-red-500">*</span></label>
                    <input {...createForm.register('pin')} type="text" inputMode="numeric" maxLength={4} placeholder="4 digits"
                      className="w-full h-10 px-3 text-sm border border-gray-200 rounded-lg outline-none transition focus:ring-2 focus:ring-red-100 tracking-widest" />
                    {createForm.formState.errors.pin && <p className="text-red-500 text-xs">{createForm.formState.errors.pin.message}</p>}
                  </div>
                </div>
              </>
            )}
            ...
            {/* Email */}
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Email <span className="text-gray-400 font-normal normal-case">(optional)</span></label>
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Role <span className="text-red-500">*</span></label>
              ...
              {'role' in form.formState.errors && <p className="text-red-500 text-xs">{form.formState.errors.role?.message}</p>}
            </div>
          </div>
          ...
          >
            ...
            <button type="submit" disabled={mutation.isPending}
              className="flex-1 h-10 text-sm font-semibold text-white rounded-lg disabled:opacity-60 transition-opacity"
              style={{ backgroundColor: 'var(--color-primary)' }}>
...
// ── Reset PIN Dialog ──────────────────────────────────────────────────────────
function ResetPinPanel({ open, onClose, user }: { open: boolean; onClose: () => void; user?: UserRecord }) {
  const qc = useQueryClient()
      ...
      <div className="relative bg-white w-[440px] h-full shadow-2xl flex flex-col">
        ...
        <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 px-5 py-5 space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">New PIN <span className="text-red-500">*</span></label>
              <input {...register('pin')} type="text" inputMode="numeric" maxLength={4} placeholder="4-digit PIN"
                className="w-full h-10 px-3 text-sm border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-red-100 tracking-widest" />
          ...
          <div className="px-5 py-4 flex gap-3 shrink-0" style={{ borderTop: '1px solid rgba(0,0,0,0.07)' }}>
            ...
            <button type="submit" disabled={mutation.isPending}
              className="flex-1 h-10 text-sm font-semibold text-white rounded-lg disabled:opacity-60"
              style={{ backgroundColor: 'var(--color-primary)' }}>
...
export function UsersPage() {
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      />
      <UserFormPanel open={dialogOpen} onClose={() => setDialogOpen(false)} user={editUser} />
      <ResetPinPanel open={!!resetUser} onClose={() => setResetUser(undefined)} user={resetUser} />
    </div>
  )
}
